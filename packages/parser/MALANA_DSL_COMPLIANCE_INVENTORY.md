...
matcher's traversal policy. The per-message call in `zt1.f` passes
`g40.d0.q()` into `g40.d0.N(...)`. That hardcoded list contains 13 names,
including `GRM_VOID`, but it is not the seed's exact category set: it includes
nonexistent `GRM_BLACKLIST` and omits real `GRM_APPOINTMENT`. The APK's bundled
seed and this repository's seed are semantically identical (`VERSION 1.1.36`),
so this is not asset-version skew. `d61.baz.l(...)` skips a name absent from
the compiled category map; consequently the default list can contribute 12
real categories, while `GRM_APPOINTMENT` is not selected through this path.
Whether every combined branch is attempted for every message, or a later
runtime gate narrows it, remains open until `g40.d0.N`/the matcher call path is
traced.
