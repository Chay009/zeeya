...
export class YesBankParser extends BankParser {
  ...
  protected override extractTransactionType(message: string): TransactionType | null {
    ...
    if (lower.includes('spent') && lower.includes('yes bank card') && lower.includes('avl lmt')) return 'CREDIT';
  ...
  protected override isTransactionMessage(message: string): boolean {
    const lower = message.toLowerCase();
    if (lower.includes('otp') || lower.includes('verification') || lower.includes('one time password')) return false;
    if (lower.includes('offer') || lower.includes('cashback offer') || lower.includes('discount')) return false;
    const keywords = ['spent on yes bank card','debited','credited','withdrawn','deposited','avl lmt'];
