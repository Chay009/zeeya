...
export class AxisBankParser extends BankParser {
  ...
  protected extractMerchant(message: string, sender: string): string | null {
    ...
    const debitCardM = /debited from A\/c no\. [^\s]+ on ([^0-9]+?)(?:\d{2}-\d{2}-\d{4})/i.exec(message);
    if (debitCardM) {
      const merchant = this.cleanMerchantName(debitCardM[1]!.trim());
      if (this.isValidMerchantName(merchant)) return merchant;
    }

    const spentIstM = /Spent[\s\S]*?IST\s*\n\s*([^\n]+?)(?:\s*\n|\s*Avl Limit:|\s*Avl Lmt|\s*Not you?)/i.exec(message);
    if (spentIstM) {
    ...
    const spentTimeM = /Spent[\s\S]*?\d{2}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s*\n\s*([^\n]+?)(?:\s*\n|\s*Avl Limit:|\s*Avl Lmt|\s*Not you?)/i.exec(message);
    if (spentTimeM) {
