...
export class KotakBankParser extends BankParser {
  ...
  protected override isTransactionMessage(message: string): boolean {
    ...
    if (
      ...
      lower.includes('verification code') || lower.includes('offer') ||
      lower.includes('discount') || lower.includes('cashback offer') || lower.includes('win ')
    ) return false;
    if (
      lower.includes('has requested') || lower.includes('payment request') ||
      lower.includes('collect request') || lower.includes('requesting payment') ||
      lower.includes('requests rs') || lower.includes('ignore if already paid')
    ) return false;
    const keywords = ['sent','debited','credited','withdrawn','deposited','spent','received','transferred','paid'];
