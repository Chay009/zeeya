...
interface TSToken { type: string; value: string; raw: string }
...
describe('Yuga FSA — Java vs TypeScript port', () => {
  ...
  it('token sequences match for every corpus message', () => {
    for (let idx = 0; idx < CORPUS.length; idx++) {
      ...
      for (let t = 0; t < java.length; t++) {
        ...
        const valueMatch = isDateToken(jt.type) || isDateToken(tt.type)
          ? true
          : jt.value === tt.value;
    ...
    const total  = CORPUS.reduce((n, _, i) => n + Math.max(javaResults[i]!.length, tsTokenize(CORPUS[i]!).length), 0);
  ...
  it('prints a per-message diff table', () => {
    ...
    for (let idx = 0; idx < CORPUS.length; idx++) {
      ...
      const match = java.length === ts.length && java.every((jt, k) => {
        const tt = ts[k]!;
