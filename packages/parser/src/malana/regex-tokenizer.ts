...
class FsaContextMap {
  ...
  putChar(key: string, ch: string) {
    this.put(key, ch);
  }

  getType(): string {
    return this._map.get(TY_TYP) ?? "";
  }
  ...
  setVal(name: string, val: string) {
    this._valMap.set(name, val);
  }
  getVal(name: string) {
    return this._valMap.get(name);
  }
  getValMap() {
    return this._valMap;
  }
  getIndex() {
    return parseInt(this._map.get(INDEX_KEY) ?? "0");
  }
  setIndex(index: number) {
    this._map.set(INDEX_KEY, String(index));
  }
  ...
  upgrade(value: string) {
    switch (this.prevKey) {
      case DT_HH:
        this.put(DT_mm, value);
        this.prevKey = DT_mm;
        break;
      case DT_mm:
        this.put(DT_ss, value);
        this.prevKey = DT_ss;
        break;
      case DT_D:
        this.put(DT_MM, value);
    ...
    try {
      ...
      if (this._map.has(DT_MM)) month = parseInt(this._map.get(DT_MM)!);
        ...
        const mi = [
          ...
          "november",
          "december",
        ].findIndex((m) => mname.startsWith(m.slice(0, 3)));
        if (mi >= 0) month = mi + 1;
      }
...
function seeding(seed: string, root: GenTrie) {
  for (const word of seed.split(",")) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      } else if (i < len - 1 && word.charCodeAt(i + 1) === 59) {
        // semicolon
        t.leaf = true;
        t.token = word.replace(/;/g, "");
        i++; // skip semicolon
...
function isNumStr(s: string) {
  return s.length > 0 && [...s].every(isNum);
}
function isDateOp(c: string) {
  const n = c.charCodeAt(0);
  return n === CH_SLSH || n === CH_HYPH || n === CH_SPACE;
}
function isDelimiter(c: string) {
  const n = c.charCodeAt(0);
  return n === CH_SPACE || n === CH_FSTP || n === CH_COMA || n === CH_RBKT || n === CH_NLINE;
}
function isTimeOp(c: string) {
  return c.charCodeAt(0) === CH_COLN;
}
...
function checkTypes(root: RootTrie, type: string, word: string): [number, string] | null {
  ...
  for (; i < word.length; i++) {
    const ch = charAt(word, i);
    if (t.leaf && !t.next.has(ch) && isTypeEnd(ch)) return [i - 1, t.token];
...
function lookAheadForMerid(str: string, index: number): boolean {
  ...
  for (let i = index + 1; i < index + 4; i++) if (meridienTimeAhead(str, i)) return true;
  return false;
}
...
function setIfNumRange(str: string, i: number, map: FsaContextMap) {
  ...
  if (isDelimiter(trimmed[trimmed.length - 1]!)) trimmed = trimmed.slice(0, -1);
...
function accAmtNumPct(
  ...
  if (n === CH_FSTP) {
    ...
    subStr.length > 10 &&
    i + 1 < str.length &&
    str.charCodeAt(i + 1) !== CH_STAR &&
    nextSpace(subStr) - i > 12
  ) {
    ...
    n === CH_PCT ||
    (n === CH_SPACE && i + 1 < str.length && str.charCodeAt(i + 1) === CH_PCT)
  ) {
...
function parseInternal(str: string, config: Map<string, string>): [number, FsaContextMap] | null {
    ...
    switch (state) {
      ...
      case 3:
        if (isNum(c)) {
          map.append(c);
          state = 8;
        } else if (cn === CH_SPACE && hasISDCodePrefix(str, i)) {
          /* stay 3 */
        } else if (cn === CH_SPACE && i === str.length - 1) {
          state = -1;
        } else if (isTimeOp(c)) {
          delimiterStack.push(c);
          map.setType(TY_DTE, DT_HH);
      ...
      case 6:
        if (isNum(c)) {
          map.upgrade(c);
          if (i + 1 < str.length && isNum(charAt(str, i + 1))) map.append(charAt(str, i + 1));
          i++;
          state = -1;
        } else state = -1;
        break;
      ...
      case 11:
        if (cn === 42 || cn === 88 || cn === 120) map.append("X");
          ...
          cn === CH_SPACE &&
          i + 1 < str.length &&
          (str.charCodeAt(i + 1) === 42 ||
            str.charCodeAt(i + 1) === 88 ||
          ...
          i = isNum(charAt(str, insi)) ? insi - 1 : insi;
        } else {
          i = i - 1;
          state = -1;
        }
      ...
      case 12: {
        if (isNum(c)) {
          if (
            (i > 2 && charAt(str, i - 1) === " " && isNum(charAt(str, i - 2))) ||
            delimiterStack.pop() === "/"
          ) {
            map.append(c);
          ...
          map.append(c);
          state = 10;
        } else if (cn === CH_HYPH && i + 1 < str.length && isNum(charAt(str, i + 1))) {
          state = 39;
        } else if (
          getPrevState(prevStates) === 37 &&
          cn === CH_HYPH &&
          ...
          else if (i - 3 > 0 && charAt(str, i - 3) === "," && comma_count === 1) {
            const c1 = map.pop();
            const c2 = map.pop();
            map.append(".");
            map.append(c2);
      ...
      case 13:
        if (isNum(c)) map.append(c);
          ...
          cn === CH_HASH &&
          (i === str.length - 1 || (i + 1 < str.length && isDelimiter(charAt(str, i + 1))))
        ) {
          map.setType(TY_USSD);
        } else {
          i = i - 1;
      ...
      case 14:
        if (isNum(c)) map.append(c);
          ...
          map.get(TY_AMT) != null &&
          (map.get(TY_AMT) ?? "").includes(".") &&
          possibleTimeAhead(str, i)
        ) {
          const samt = map.get(TY_AMT) ?? "";
          // samt.includes('.') just verified above -> split('.') has >= 2 parts.
          const [hh0, mm0] = samt.split(".") as [string, string, ...string[]];
          map.put(DT_HH, hh0);
          map.put(DT_mm, mm0);
          map.setType(TY_DTE);
          state = 7;
          ...
          map.setType(TY_PCT, TY_PCT);
          state = -1;
        } else if ((c === "k" || c === "c") && i + 1 < str.length && charAt(str, i + 1) === "m") {
          map.setType(TY_DST, TY_DST);
          i++;
          state = -1;
        } else if ((c === "k" || c === "m") && i + 1 < str.length && charAt(str, i + 1) === "g") {
          map.setType(TY_WGT, TY_WGT);
          i++;
          state = -1;
        } else {
          if (cn === CH_FSTP && i + 1 < str.length && isNum(charAt(str, i + 1))) {
            const samt = map.get(map.getType()) ?? "";
            if (samt.includes(".")) {
              const parts = samt.split(".");
              if (parts.length === 2) {
                // parts.length === 2 just verified above.
                const [p0, p1] = parts as [string, string];
                const d = parseStrToInt(p0),
                  mm = parseStrToInt(p1);
      ...
      case 16:
        if (isNum(c)) {
          map.upgrade(c);
          state = 17;
        } else if (cn === CH_SPACE || cn === CH_COMA) {
          /* stay 16 */
        } else if ((p = checkTypes(ROOT, "FSA_MONTHS", str.substring(i))) !== null) {
          map.put(DT_MMM, p[1]);
          i += p[0];
          ...
          if (
            ...
            i + 1 < str.length &&
            (isNum(charAt(str, i + 1)) ||
              checkTypes(ROOT, "FSA_MONTHS", str.substring(i + 1)) !== null)
          ) {
            // stay 16
          } else {
            map.setType(TY_NUM, TY_NUM);
            let j = i;
            while (j >= 0 && j < str.length && !isNum(charAt(str, j))) j--;
            i = j;
            state = -1;
          }
        }
      ...
      case 17:
        if (isNum(c)) {
          ...
          delimiterStack.push(c);
          state = 19;
        } else if (cn === CH_COMA && delimiterStack.pop() === ",") {
          map.setType(TY_NUM, TY_NUM);
          state = 12;
        } else if (cn === CH_FSTP && delimiterStack.pop() === ",") {
          map.setType(TY_NUM, TY_NUM);
          map.append(c);
          state = 10;
        } else {
      ...
      case 18:
        if (isDateOp(c)) {
          delimiterStack.push(c);
          state = 19;
        } else if (isNum(c) && delimiterStack.pop() === ",") {
          map.setType(TY_NUM, TY_NUM);
          state = 12;
          map.append(c);
        } else if (isNum(c) && delimiterStack.pop() === "-") {
          ...
          map.append(c);
          state = 10;
        } else if (cn === CH_FSTP && map.contains(DT_D) && map.contains(DT_MM)) {
          state = -1;
        } else {
          map.setType(TY_STR, TY_STR);
          i = i - 1;
      ...
      case 24:
        ...
        } else if (cn === CH_SQOT && i + 1 < str.length && isNum(charAt(str, i + 1))) {
          /* stay 24 */
        } else if (c === "|") {
          /* stay 24 */
        } else {
      ...
      case 27:
        if (isDateOp(c)) {
          ...
          if (
            ...
            (i + 2 === str.length ||
              (i + 2 < str.length &&
                (isDelimiter(charAt(str, i + 2)) || charAt(str, i + 2) === "/"))) &&
            checkTimeRange
          ) {
            map.setType(TY_TMS, TY_TMS);
            map.append(charAt(str, i + 1));
            i++;
            state = -1;
          } else if (prevDelim === " ") {
            state = 41;
      ...
      case 28:
        if (isNum(c) && map.getType() !== TY_TAGNUM) {
          map.put(DT_D, c);
          state = 29;
        } else {
          map.setType(TY_STR, TY_STR);
      ...
      case 30:
        if (cn === CH_COMA || cn === CH_SPACE || cn === CH_NLINE) {
          /* stay 30 */
        } else if (isNum(c)) {
          map.put(DT_D, c);
          state = 31;
      ...
      case 32:
        if ((p = checkTypes(ROOT, "FSA_MONTHS", str.substring(i))) !== null) {
          ...
          i += p[0];
          state = 24;
        } else if (cn === CH_COMA || cn === CH_SPACE || cn === CH_NLINE) {
          /* stay 32 */
        } else if ((p = checkTypes(ROOT, "FSA_DAYSFFX", str.substring(i))) !== null) {
          i += p[0];
        } else {
      ...
      case 33:
        if (i + 3 < str.length && isNum(c) && str.substring(i + 1, i + 3) === "th") {
          ...
          map.put(DT_D, c);
          state = 34;
        } else if (cn === CH_SPACE || cn === CH_COMA || cn === CH_HYPH) {
          /* stay 33 */
        } else if (
          getPrevState(prevStates) === 1 &&
          cn === CH_FSTP &&
          lookAheadForNum(str, i) !== -1
        ) {
      ...
      case 36:
        if (isNum(c)) {
          map.append(c);
          counter++;
        } else if (cn === CH_FSTP && i + 1 < str.length && isNum(charAt(str, i + 1))) {
          map.append(c);
          state = 10;
        } else if (cn === CH_HYPH && i + 1 < str.length && isNum(charAt(str, i + 1))) {
          delimiterStack.push(c);
      ...
      case 41:
        if (isNum(c)) map.append(c);
          ...
          if (i >= 11 && i + 1 < str.length && isNum(charAt(str, i + 1))) {
            state = -1;
            i--;
          }
          // else stay 41
      ...
      case 45:
        ...
        else if (cn === CH_SPACE && i + 1 < str.length && isNum(charAt(str, i + 1))) {
          /* stay 45 */
        } else {
          if (i - 1 > 0 && charAt(str, i - 1) === ",") i = i - 2;
          else i = i - 1;
  ...
  if (map.getType() === TY_AMT) {
    ...
    if (
      !amt ||
      (amt.includes(".") && amt.split(".")[0]!.length > 8) ||
  ...
  if (map.getType() === TY_NUM) {
    ...
    if (
      ...
      if (numVal) {
        ...
        if (numVal.length === 10 && "6789".includes(numVal[0]!)) map.setVal("num_class", TY_PHN);
        else if (numVal.length === 12 && numVal.startsWith("91")) map.setVal("num_class", TY_PHN);
        else if (numVal.length === 11 && numVal.startsWith("18")) map.setVal("num_class", TY_PHN);
        else if (numVal.length === 11 && numVal[0]! === "0") map.setVal("num_class", TY_PHN);
      }
    }
    ...
    if (ind >= 0 && ind < str.length) {
      if (
        isNum(charAt(str, ind)) ||
        checkTypes(ROOT, "FSA_MONTHS", sub) ||
        checkTypes(ROOT, "FSA_DAYS", sub)
      ) {
        ...
        if (pTime) {
          ...
          if (
            iTime < str.length &&
            (isNum(charAt(str, iTime)) ||
              checkTypes(ROOT, "FSA_DAYS", str.substring(iTime)) !== null)
          ) {
...
function prepareResult(
  ...
  return {
    type,
    value: index >= 0 && index <= str.length ? str.substring(0, index) : "",
    hasTime: false,
  };
}
