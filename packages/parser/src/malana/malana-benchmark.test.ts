...
function parseAmount(s: string): number {
  ...
  return parseFloat(
    s
      .replace(/[₹$€¥₩£]|Rs\.?|INR|USD|EUR|GBP|AED|SGD\s*/gi, "")
      .replace(/,/g, "")
      .trim(),
  );
}
...
describe("Malana accuracy benchmark", () => {
  ...
  it("full benchmark report", () => {
    ...
    expect(acctPass / n, `Account accuracy below 90%: ${acctPass}/${n}`).toBeGreaterThanOrEqual(
      0.9,
    );
  });
  ...
  it("does not parse non-transaction SMS", () => {
    ...
    expect(failures.length, `Too many false positives: ${failures.join(", ")}`).toBeLessThanOrEqual(
      1,
    );
  });
});
