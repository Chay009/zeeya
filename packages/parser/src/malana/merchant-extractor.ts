...
function isEntirelyStructured(candidate: string, currencyRegistry: CurrencyRegistry): boolean {
  const tokens = regexTokenize(candidate, currencyRegistry);
...
export function isValidMerchantCandidate(
  candidate: string,
  ...
  if (isEntirelyStructured(candidate, currencyRegistry)) return false; // date / amount / number
...
export function extractMerchantWithAnchors(
  ...
  for (const anchor of anchorsInPriorityOrder(anchors)) {
    ...
    if (isValidMerchantCandidate(candidate, currencyRegistry)) return candidate;
  }
  return null;
}
...
export function extractRawMerchant(
  message: string,
  currencyRegistry: CurrencyRegistry,
  bankName?: string | null,
): string | null {
  return extractMerchantWithAnchors(message, prioritizedAnchors, currencyRegistry, bankName);
}
