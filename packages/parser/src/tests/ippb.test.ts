...
describe('IPPBParser', () => {
  ...
  it('parses debit transaction with UPI VPA', () => {
    const msg =
      'Your A/C X1234 Debit Rs.500.00 to john@upi. Avl Bal Rs.1000.00 Ref 560002638161';
  ...
  it('parses debit transaction to plain merchant name (no UPI @)', () => {
    ...
    const msg =
      'Your A/C X9012 Debit Rs.200.00 to SOMEMERCHANT Avl Bal Rs.3000.00 Ref 987654321';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('returns UPI Payment as merchant when "for upi" present but no specific merchant', () => {
    const msg =
      'Your A/C X3456 Debit Rs.300.00 for UPI payment. Avl Bal Rs.700.00 Ref 111222333';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('parses credit via Info: UPI/CREDIT pattern', () => {
    const msg =
      'A/C X7890 credit Rs.2500.00. Avl Bal Rs.10000.00. Info: UPI/CREDIT/998877665544';
  ...
  it('extracts amount from Rs. (with space) pattern', () => {
    const msg =
      'Your A/C X1111 Debit Rs. 750.50 to vendor@pay. Avl Bal Rs. 250.00 Ref 555666777';
  ...
  it('extracts last 4 digits when account number longer than 4 digits', () => {
    const msg =
      'Your A/C X123456789 Debit Rs.100.00 to pay@upi. Avl Bal Rs.900.00 Ref 100200300';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts full number when account number less than 4 digits', () => {
    const msg =
      'Your A/C X123 Debit Rs.50.00 to test@upi. Avl Bal Rs.450.00 Ref 400500600';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('handles A/C without X prefix', () => {
    const msg =
      'Your A/C 4321 Debit Rs.250.00 to shop@pay. Avl Bal Rs.750.00 Ref 700800900';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts balance from Avl Bal Rs. pattern', () => {
    const msg =
      'Your A/C X2222 Debit Rs.100.00 to abc@def. Avl Bal Rs.436.91 Ref 111000111';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts balance from Avl Bal Rs. with comma-formatted amount', () => {
    const msg =
      'Your A/C X3333 Debit Rs.500.00 to shop@pay. Avl Bal Rs.12,345.67 Ref 222333444';
    const r = parser.parse(msg, 'AX-IPBMSG-T', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts reference from Ref pattern', () => {
    const msg =
      'Your A/C X4444 Debit Rs.100.00 to vendor@upi. Avl Bal Rs.900.00 Ref 560002638161';
    const r = parser.parse(msg, 'CP-IPBMSG-S', 0);
    expect(r).not.toBeNull();
    expect(r!.reference).toBe('560002638161');
  });
  ...
  it('identifies "debit rs" messages as transactions', () => {
    const msg =
      'Your A/C X6666 Debit Rs.200.00 to someone@upi. Avl Bal Rs.800.00 Ref 999888777';
  ...
  it('does not parse OTP messages', () => {
    const msg =
      'Your OTP for IPPB login is 123456. Do not share this with anyone.';
    expect(parser.parse(msg, 'CP-IPBMSG-S', 0)).toBeNull();
  });

  it('does not parse promotional / offer messages', () => {
    const msg =
      'Exciting offer! Get cashback offer on IPPB transactions. T&C apply.';
    expect(parser.parse(msg, 'CP-IPBMSG-S', 0)).toBeNull();
  });

  it('does not parse messages without an amount', () => {
    const msg =
      'Your A/C X8888 has been debited. Contact IPPB for details.';
    expect(parser.parse(msg, 'CP-IPBMSG-S', 0)).toBeNull();
  });
});
