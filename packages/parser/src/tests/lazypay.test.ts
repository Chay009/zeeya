...
describe('LazyPayParser', () => {
  ...
  it('rejects failed payment messages', () => {
    const msg =
      'Payment of Rs. 100.00 could not be processed due to a failure. Please try again.';
    expect(parser.parse(msg, 'BP-LZYPAY-S', 0)).toBeNull();
  });

  it('rejects unsuccessful messages', () => {
    const msg =
      'Your transaction of Rs. 200.00 was unsuccessful. Please retry.';
    expect(parser.parse(msg, 'BP-LZYPAY-S', 0)).toBeNull();
  });
  ...
  it('strips Pvt Ltd suffix from merchant', () => {
    const msg =
      'Payment of Rs. 299.00 for txn TXN777888999 on XYZ Pvt Ltd was successful.';
    const r = parser.parse(msg, 'BP-LZYPAY-S', 0);
    expect(r).not.toBeNull();
