...
describe('AirtelPaymentsBankParser', () => {
  ...
  it('filters out masked Txn IDs containing x', () => {
    const msg =
      'Rs. 5.00 debited from Airtel Payments Bank a/c Txn ID xxxxxxxx Bal:15.56';
    const r = parser.parse(msg, 'AD-AIRBNK-S', 0);
    expect(r).not.toBeNull();
  ...
  it('returns "Airtel Payments Bank Transaction" when message contains "airtel payments bank"', () => {
    const msg =
      'Airtel Payments Bank a/c is credited with Rs.20.00. Txn ID: 560992310006.';
    const r = parser.parse(msg, 'AD-AIRBNK-S', 0);
    expect(r).not.toBeNull();
  ...
  it('detects INCOME from "is credited" keyword', () => {
    const msg =
      'Airtel Payments Bank a/c is credited with Rs.50.00. Txn ID: 100200300400.';
    const r = parser.parse(msg, 'AD-AIRBNK-S', 0);
    expect(r).not.toBeNull();
    expect(r!.type).toBe('INCOME');
  });
  ...
  it('detects EXPENSE from "debited from" keyword', () => {
    const msg =
      'Rs. 75.00 debited from Airtel Payments Bank a/c Txn ID 200300400500 Bal:425.00';
    const r = parser.parse(msg, 'AD-AIRBNK-S', 0);
    expect(r).not.toBeNull();
    expect(r!.type).toBe('EXPENSE');
  });
