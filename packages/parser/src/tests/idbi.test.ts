...
describe('IDBIBankParser', () => {
  ...
  it('parses debit "debited with Rs" with comma-separated amount', () => {
    const msg =
      'Your account has been debited with Rs 1,500.00. Bal Rs 12,000.00';
  ...
  it('parses UPI debit "debited for Rs" pattern with account and RRN', () => {
    const msg =
      'IDBI Bank Acct XX1234 debited for Rs 1040.00. RRN 519766155631. Bal Rs 5000.00';
    const r = parser.parse(msg, 'CP-IDBIBK-S', 0);
    expect(r).not.toBeNull();
  ...
  it('parses debit "debited for Rs" with UPI reference', () => {
    const msg =
      'IDBI Bank Acct XX5678 debited for Rs 250.00. UPI:521687538121. Bal Rs 8750.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('parses credit "credited with Rs" pattern', () => {
    const msg =
      'Your IDBI Bank account has been credited with Rs 5000.00. Bal Rs 15000.00';
    const r = parser.parse(msg, 'CP-IDBIBK-S', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts merchant from "; <merchant> credited." pattern', () => {
    const msg =
      'IDBI Bank Acct XX3456 credited with Rs 10000.00; SALARY credited. Bal Rs 50000.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts account last4 from "Acct XX1234" pattern', () => {
    const msg =
      'IDBI Bank Acct XX1111 debited for Rs 100.00. Bal Rs 900.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts account last4 from "IDBI Bank Acct XX1234" pattern', () => {
    const msg =
      'IDBI Bank Acct XX2222 credited with Rs 500.00. Bal Rs 5500.00';
  ...
  it('extracts RRN reference', () => {
    const msg =
      'IDBI Bank Acct XX3333 debited for Rs 750.00. RRN 987654321098. Bal Rs 3250.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts UPI reference', () => {
    const msg =
      'IDBI Bank Acct XX4444 debited for Rs 120.00. UPI:112233445566. Bal Rs 1880.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('extracts balance from "Bal Rs" pattern', () => {
    const msg =
      'IDBI Bank Acct XX5555 debited for Rs 200.00. Bal Rs 3694.38';
    const r = parser.parse(msg, 'CP-IDBIBK-S', 0);
    expect(r).not.toBeNull();
    expect(r!.balance).toBe(3694.38);
  });
  ...
  it('extracts balance from "Bal Rs" with comma-separated amount', () => {
    const msg =
      'Your account debited with Rs 100.00. Bal Rs 1,23,456.78';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
  ...
  it('does not parse OTP messages', () => {
    const msg =
      'Your IDBI Bank OTP is 987654. Valid for 10 minutes. Do not share with anyone.';
  ...
  it('returns correct currency INR', () => {
    const msg =
      'IDBI Bank Acct XX6666 debited for Rs 50.00. Bal Rs 950.00';
    const r = parser.parse(msg, 'IDBIBK', 0);
    expect(r).not.toBeNull();
    expect(r!.currency).toBe('INR');
  });
  ...
  it('sets sender and timestamp on parsed result', () => {
    const msg =
      'IDBI Bank Acct XX7777 debited with Rs 300.00. Bal Rs 700.00';
