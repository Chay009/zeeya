...
describe("CentralBankOfIndiaParser", () => {
  ...
  it('parses credit with "Rs.XXX credited" format', () => {
    const msg = "Rs.1500.00 credited to A/C ending 7890. Total Bal Rs.7000.00 CR -CBoI";
  ...
  it('parses debit with "Rs.XXX debited" format', () => {
    const msg = "Rs.250.00 debited from A/C ending 7890. Clear Bal Rs.3000.00 CR -CBoI";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
  ...
  it("returns negative balance when DR suffix is present", () => {
    const msg = "Your account XX1234 Debited by Rs.200.00. Total Bal Rs.150.00 DR. -CBoI";
    const r = parser.parse(msg, "CBOI", 0);
    expect(r).not.toBeNull();
  ...
  it("parses balance from Clear Bal pattern with CR", () => {
    const msg = "A/C XX5678 Credited by Rs.300.00. Clear Bal Rs.1200.00 CR. -CBoI";
    const r = parser.parse(msg, "BV-CENTBK-S", 0);
    expect(r).not.toBeNull();
  ...
  it('returns "UPI Credit" merchant when message says credited via UPI', () => {
    const msg = "Rs.800.00 credited to account XX4321 via UPI. Total Bal Rs.9000.00 CR -CBoI";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
  ...
  it('returns "UPI Payment" merchant when message says debited via UPI', () => {
    const msg = "Rs.400.00 debited from account XX4321 via UPI. Total Bal Rs.8600.00 CR -CBoI";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
  ...
  it('extracts account last4 from "A/C ending" pattern', () => {
    const msg = "Rs.600.00 credited to A/C ending 5566. Total Bal Rs.6600.00 CR -CBoI";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
  ...
  it("maps withdrawn to EXPENSE type", () => {
    const msg = "Rs.2000.00 withdrawn from account XX6677. Avl Bal Rs.3000.00";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
    expect(r!.type).toBe("EXPENSE");
  });
  ...
  it("maps deposited to INCOME type", () => {
    const msg = "Rs.3000.00 deposited to account XX6677. Avl Bal Rs.8000.00";
    const r = parser.parse(msg, "CENTBK", 0);
    expect(r).not.toBeNull();
    expect(r!.type).toBe("INCOME");
  });
