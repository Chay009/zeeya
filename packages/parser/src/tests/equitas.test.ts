...
describe("EquitasBankParser", () => {
  ...
  describe("UPI debit (INR X debited)", () => {
    const message = "INR 500.00 debited on 01-01-25 to SWIGGY. Avl Bal is INR 1,000.00";
  ...
  describe("UPI credit (INR X credited)", () => {
    const message = "INR 1,000.00 credited on 15-01-25 from AMAZON. Avl Bal is INR 5,000.00";
    const result = parser.parse(message, "AD-EQUITAS", 0);
  ...
  describe("Offer message is rejected", () => {
    it("returns null", () => {
      expect(
        parser.parse(
          "Special offer for Equitas Bank customers! Get 10% cashback offer.",
          "EQUTAS",
          0,
        ),
      ).toBeNull();
