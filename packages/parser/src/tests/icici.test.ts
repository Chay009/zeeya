...
describe('ICICIBankParser', () => {
  ...
  it('should not parse future debit variations', () => {
    expect(
      parser.parse('Rs. 500.00 will be debited from your account on 05-Oct-25 for EMI payment', 'AX-ICICIT-S', 0),
    ).toBeNull();
    expect(
      parser.parse('Your ICICI Bank Account will be debited with Rs 1,000.00 on 10-Oct-25', 'AX-ICICIT-S', 0),
    ).toBeNull();
    expect(
      parser.parse('AutoPay: Rs 299.00 will be debited on 15-Oct-25 for Spotify subscription', 'AX-ICICIT-S', 0),
    ).toBeNull();
  });
