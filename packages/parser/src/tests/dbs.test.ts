...
describe("DBSBankParser", () => {
  ...
  it("parses INR amount debited (amount before keyword)", () => {
    const msg = "INR 500 debited from your DBS Bank account ****3456. Balance: INR 2000.00";
    const r = parser.parse(msg, "CP-DBSBNK-S", 0);
    expect(r).not.toBeNull();
  ...
  it("parses INR amount credited (amount before keyword)", () => {
    const msg = "INR 1000 credited to your DBS Bank account ****2222. Balance: INR 6000.00";
  ...
  it("parses deposited transaction", () => {
    const msg = "INR 3000.00 deposited to your DBS Bank account ****8888. Balance: INR 11000.00";
  ...
  it('extracts account last4 from "a/c ****NNNN" pattern', () => {
    const msg = "DBS a/c ****2222 debited with INR 75. Balance: INR 925.00";
    const r = parser.parse(msg, "DBSBNK", 0);
    expect(r).not.toBeNull();
    expect(r!.accountLast4).toBe("2222");
  });
  ...
  it('extracts account last4 from "account ****NNNN" pattern', () => {
    const msg = "DBS account ****3333 debited with INR 200. Current Balance is INR 800.00";
    const r = parser.parse(msg, "DBS", 0);
    expect(r).not.toBeNull();
  ...
  it('extracts balance from "Balance: INR" pattern', () => {
    const msg = "Your DBS account ****5555 credited with INR 500. Balance: INR 1000";
    const r = parser.parse(msg, "DBS", 0);
    expect(r).not.toBeNull();
  ...
  it('extracts balance from "Avl Bal: INR" pattern', () => {
    const msg = "Your DBS account ****6666 debited with INR 200. Avl Bal: INR 4800.00";
    const r = parser.parse(msg, "DBSBNK", 0);
    expect(r).not.toBeNull();
  ...
  it("does not parse messages without amount", () => {
    const msg =
      "Your DBS Bank account no ****1234 has been debited. Please contact us for details.";
    expect(parser.parse(msg, "DBSBNK", 0)).toBeNull();
  });
});
