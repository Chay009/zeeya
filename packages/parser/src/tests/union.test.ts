...
describe("UnionBankParser", () => {
  ...
  it("extracts amount from Rs: (colon) format", () => {
    const msg = "A/c *5678 Credited Rs:500.00 by NEFT ref no 200000001234 Avl Bal Rs:99999.00";
  ...
  it("extracts amount from INR format when no Rs prefix is present", () => {
    ...
    const msg = "A/c X9999 Debited INR 2500 on 10-01-2025. Available Balance INR 50000";
  ...
  it("extracts balance from Avl Bal Rs: pattern", () => {
    const msg = "A/c *2222 Debited Rs:999.00 by Mob Bk ref no 555666777 Avl Bal Rs:88888.88";
  ...
  it('extracts reference from "ref no" pattern', () => {
    const msg = "A/c *3333 Debited Rs:750.00 by Mob Bk ref no 123000000456 Avl Bal Rs:10000.00";
    const r = parser.parse(msg, "AX-UNIONB-S", 0);
    expect(r).not.toBeNull();
  ...
  it("extracts reference from txn pattern", () => {
    const msg = "A/c *6666 Debited Rs:200.00 Txn:TXN20240101 Avl Bal Rs:5000.00";
    const r = parser.parse(msg, "UNIONB", 0);
    expect(r).not.toBeNull();
  ...
  it("parses credit transaction type correctly", () => {
    const msg = "A/c *7777 Credited Rs:1000.00 by NEFT ref no 777888999 Avl Bal Rs:20000.00";
