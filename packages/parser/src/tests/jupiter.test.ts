...
describe("JupiterBankParser", () => {
  ...
  it('parses "jupiter csb edge" transaction as Credit Card Payment', () => {
    const msg = "Rs.200.00 debited via Jupiter CSB Edge account. Avl Bal: Rs.3000.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
  ...
  it("parses generic credit card debit", () => {
    const msg = "Rs.500.00 debited to your Jupiter credit card ending 9999. Avl Bal: Rs.7500.00";
  ...
  it("parses UPI debit with reference number", () => {
    const msg = "Rs.500.00 debited via UPI Ref no.281751568470. Avl Bal: Rs.4500.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
    expect(r!.amount).toBe(500);
    expect(r!.type).toBe("EXPENSE");
  ...
  it("parses UPI credit with reference number", () => {
    const msg = "Rs.2000.00 credited via UPI Ref no.123456789012. Avl Bal: Rs.12000.00";
  ...
  it("parses UPI Ref no without period", () => {
    const msg = "Rs.300.00 debited via UPI Ref no 987654321098. Avl Bal: Rs.1200.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
  ...
  it("parses account credit (non-UPI, non-card)", () => {
    const msg = "Rs.5000.00 credited to your Jupiter account. Avl Bal: Rs.20000.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
  ...
  it('extracts account last4 from "Card ending NNNN" pattern', () => {
    const msg = "Rs.250.00 debited to your Card ending 4321. Avl Bal: Rs.8000.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
  ...
  it("extracts UPI reference number", () => {
    const msg = "Rs.100.00 debited via UPI Ref no.999888777666. Avl Bal: Rs.900.00";
    const r = parser.parse(msg, "AX-JTEDGE-S", 0);
    expect(r).not.toBeNull();
  ...
  it("does not parse offer/promotional messages", () => {
    const msg = "Exciting offer! Get 10% cashback offer on your Jupiter Edge card. T&C apply.";
    expect(parser.parse(msg, "AX-JTEDGE-S", 0)).toBeNull();
  });

  it("does not parse messages without a valid amount", () => {
    const msg = "Your Jupiter CSB account has been debited. Please check app for details.";
    expect(parser.parse(msg, "AX-JTEDGE-S", 0)).toBeNull();
  });
